...
class CompositionDataset(Dataset):
    def __init__(
        ...
        self.seen_mask = torch.BoolTensor([pair in seen_pairs for pair in self.pairs])

        self.objs_by_attr = {a: [] for (a, o) in self.pairs}
    ...
    def __str__(self) -> str:
        ...
        n_seen_img_val  = sum(p in set(self.train_pairs) for p in zip(attr_val, obj_val))
        _, attr_test, obj_test = zip(*self.test_data)
        n_seen_img_test = sum(p in set(self.train_pairs) for p in zip(attr_test, obj_test))
...
class CompositionDatasetEmbeddings(CompositionDataset):
    ...
    def load_text_embs(self, pairs):
        '''Loads text embeddings for the given (attr, obj) pairs.'''
        text_embs_data = torch.load(self.text_embs_path, weights_only=False)
        pair2idx = defaultdict(list)
        for i, pair in enumerate(text_embs_data['pairs']):
            pair2idx[pair].append(i)

        indices   = [i for pair in pairs for i in pair2idx[pair]]
    ...
    def load_all_image_embs(self):
        ...
        image_embs_data = torch.load(self.image_embs_path, weights_only=False)
