...
class Evaluator:
    ...
    def get_overall_metrics(self, features, all_triplets_true, topk_list=[1], progress_bar=True):  #completely changed this
        ...
        labels = torch.LongTensor(
            [self.dset.triplet2idx[triplet] for triplet in all_triplets_true]
        )
    ...
    def get_fast_metrics(self, features, all_triplets_true, topk_list=[1]):   #added third entity
        ...
        labels = torch.LongTensor(
            [self.dset.triplet2idx[triplets] for triplets in all_triplets_true]
        )
        attr1_true, attr2_true, obj_true,  = self.get_attr1_attr2_obj_from_triplets(labels)
    ...
    for e in range(config.n_exp):
        ...
        if config.experiment_name == 'clip':
            ...
            for obj in unique_objs:
                ...
                # Combine each Attr1 with the Object
                a1_embs = []
                for a1 in unique_attrs1:
                    a1_iw = factorizer.get_attr1_IW(a1)
                    a1_embs.append(factorizer.combine_ideal_words(a1_iw, obj_iw).squeeze())
                cond_attr1_embs[obj] = torch.stack(a1_embs).to(device)
                
                # Combine each Attr2 with the Object
                a2_embs = []
                for a2 in unique_attrs2:
                    a2_iw = factorizer.get_attr2_IW(a2)
                    a2_embs.append(factorizer.combine_ideal_words(a2_iw, obj_iw).squeeze())
                cond_attr2_embs[obj] = torch.stack(a2_embs).to(device)
